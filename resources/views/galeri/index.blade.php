        ...
        <div class="row g-4">
            @forelse($galleries as $g)
                @php
                    ...
                    if (!empty($g->images) && is_array($g->images) && count($g->images) > 0) {
                        $thumb = asset('storage/' . $g->images[0]);
                    }
                @endphp
                <div class="col-lg-4 col-md-6">
                    <div class="card h-100 rounded-4 overflow-hidden shadow-sm bg-white gallery-card-box border-0 position-relative" 
                         ...
                         onclick="previewImage('{{ addslashes($g->title) }}', '{{ $thumb }}', '{{ addslashes($g->description ?? '') }}')">
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content bg-dark text-white rounded-4 overflow-hidden border-0 shadow-lg" style="box-shadow: 0 25px 60px rgba(0,0,0,0.5) !important;">
            <div class="modal-header border-0 pb-0 d-flex justify-content-between align-items-center p-4">
                <h5 class="modal-title fw-bold text-white mb-0" id="galleryModalTitle"></h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body p-4 text-center">
                <div class="rounded-4 overflow-hidden bg-black d-flex align-items-center justify-content-center border border-secondary border-opacity-25" style="min-height: 400px; max-height: 75vh;">
                    <img id="galleryModalImg" src="" alt="Preview Gambar" class="img-fluid rounded" style="max-height: 72vh; object-fit: contain;">
                </div>
...
<script>
    function previewImage(title, imgUrl, desc) {
        ...
        document.getElementById('galleryModalDownload').href = imgUrl;
